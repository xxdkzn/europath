...
const RECENT = [
  { id: '1', title: 'Снял квартиру в Берлине без Schufa — делюсь опытом', cat: 'Жильё',  author: 'МК', time: '2 ч назад', replies: 34 },
  { id: '2', title: 'ВНЖ в Чехии через бизнес — полный процесс 2025',     cat: 'Визы',   author: 'АС', time: '3 ч назад', replies: 56 },
  { id: '3', title: 'Реальные зарплаты в Германии 2025 по профессиям',     cat: 'Работа', author: 'НЖ', time: '5 ч назад', replies: 92 },
  { id: '4', title: 'Wise vs Revolut vs N26 — честное сравнение',          cat: 'Банки',  author: 'ВН', time: '8 ч назад', replies: 78 },
]
    ...
    <main className="min-h-screen bg-slate-50">
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <Link href="/support" className="text-sm bg-amber-50 text-amber-700 border border-amber-200 px-3 py-1.5 rounded-lg">Поддержать ❤</Link>
        </div>
      </nav>
              ...
              {RECENT.map((t) => (
                <Link key={t.id} href={`/forum/thread/${t.id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">{t.cat} · {t.time} · {t.replies} ответов</p>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              ...
              <div className="space-y-2">
                {[['Участников','12,400'],['Тем','1,951'],['Ответов','28,340'],['Онлайн','47']].map(([l,v]) => (
            ...
            <div className="bg-amber-50 rounded-xl border border-amber-200 p-4">
              ...
              <p className="text-xs text-amber-700 mb-3">Помогите нам развиваться</p>
